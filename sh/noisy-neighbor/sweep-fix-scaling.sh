#!/bin/bash
# Fix scaling: match the unpatched matrix data points.
# 1x each: 5 villain counts × 2 working sets.
set -uo pipefail
...
run_one() {
    ...
    echo ""
    echo "RESULT: v=$villains ws=${ws_gb}G rl=$total_rl w=$total_w max_rn=${max_rn:-0}us max_wrn=${max_wrn:-0}us cmax=${cmax}ms cpu=${cpu_psi}% mem=${mem_psi}% run=$run_q blk=$blk"
    echo ""
}

for vjobs_per in 2 8 32 128 256; do
    for ws_gb in 2 32; do
        run_one $vjobs_per $ws_gb
    done
done
