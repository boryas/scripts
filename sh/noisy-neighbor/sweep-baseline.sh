#!/bin/bash
# Repeated baseline runs for two target configs.
# 10 runs each of 1024v/32G and 2048v/32G.
set -uo pipefail
...
for vjobs_per in 256; do
    villains=$((vjobs_per * 8))
    for run in $(seq 1 10); do
        echo "============================================="
        echo "=== v=$villains ws=${ws_gb}G run=$run ==="
        echo "============================================="
        ...
        for t in $(seq 5 5 $DURATION); do
            ...
            echo "POLL t=$t: cpu=$cpu_psi mem=$mem_psi pgscan=+$d_scan free=${free}MB run=$run_q cmax=${cmax}ms"
        done
        ...
        echo ""
        echo "RESULT: v=$villains ws=${ws_gb}G run=$run rl=$total_rl w=$total_w max_rn=${max_rn:-0}us max_wrn=${max_wrn:-0}us cmax=${cmax}ms"
        echo ""
    done
done
