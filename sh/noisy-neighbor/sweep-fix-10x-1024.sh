#!/bin/bash
# 10x runs of fix at 2048v/32G with reclaim workers.
set -uo pipefail
...
for run in $(seq 1 10); do
    echo "============================================="
    echo "=== FIX v=$villains ws=${ws_gb}G run=$run ==="
    echo "============================================="
    ...
    max_wrn=$(grep 'WAITERS' /tmp/monitor.log 2>/dev/null | \
        grep -oP 'max_runnable=\K[0-9]+' | sort -n | tail -1)
    # Total accumulated runnable time across all events (us)
    total_rn_us=$(grep 'RUNNABLE_LOCK' /tmp/monitor.log 2>/dev/null | \
        grep -oP 'runnable=\K[0-9]+' | awk '{s+=$1}END{print int(s/1000)}')

    echo ""
    echo "RESULT: v=$villains ws=${ws_gb}G run=$run rl=$total_rl w=$total_w max_rn=${max_rn:-0}us max_wrn=${max_wrn:-0}us cmax=${cmax}ms total_rn_ms=${total_rn_us:-0}"
    echo ""
done
