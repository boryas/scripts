...
echo "=== HOLD+SLICE: 32G/2048 ==="
mount -o noatime,commit=1 $DEV $MNT

# Start bpftrace FIRST while memory is abundant
echo "  starting bpftrace..."
bpftrace hold_and_slice.bt > /home/borisb/local/scripts/sh/noisy-neighbor/hold-slice-32g.out 2>&1 &
BT_PID=$!
...
# Let workload run, then stop bpftrace
sleep $DURATION
kill -INT $BT_PID 2>/dev/null
sleep 10
echo "  bpftrace done, checking output"
...
echo "=== OUTPUT ==="
grep -v 'WARNING\|Return value\|delete(\|SmcTiers\|RetryingSender\|ProducerWrite\|scribe_cat\|ODSBatch\|Reading tiers' \
    /home/borisb/local/scripts/sh/noisy-neighbor/hold-slice-32g.out
