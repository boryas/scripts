#!/bin/bash
# 10x runs of unpatched baseline kernel, parameterized by ws_gb and vjobs_per.
# Collects: lock monitor metrics, PSI, pgscan_direct, free memory, procs_running.
#
# Usage: $0 <ws_gb> <vjobs_per> [nruns]
#   ws_gb:      working set in GB (2 or 32)
#   vjobs_per:  fio jobs per file (8 files total, so total readers = vjobs_per * 8)
#   nruns:      number of iterations (default 10)
set -uo pipefail
...
# One-time setup: mkfs + create 8 x 4GB files
...
for run in $(seq 1 $NRUNS); do
    echo "============================================="
    echo "=== BASELINE v=$VILLAINS ws=${WS_GB}G run=$run/$NRUNS ==="
    echo "============================================="

    mount -o noatime,commit=1 $DEV $MNT

    # Snapshot pgscan_direct before
    ...
    # Poll every 5s — captures PSI, free, pgscan, procs
    ...
    echo ""
    echo "RESULT: v=$VILLAINS ws=${WS_GB}G run=$run rl=$total_rl w=$total_w max_rn=${max_rn:-0}us max_wrn=${max_wrn:-0}us cmax=${cmax}ms total_rn_ms=${total_rn_us:-0} total_wrn_ms=${total_wrn_us:-0} pgscan_d=$total_pgscan free_before=${free_before}MB free_after=${free_after}MB cpu_psi=$cpu_psi_final mem_psi=$mem_psi_final"
    echo ""
done
