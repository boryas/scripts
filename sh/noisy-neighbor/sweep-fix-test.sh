#!/bin/bash
# Test proposed fix: lock-depth gated reclaim + userspace reclaim workers.
# 3 configs: 2048v/2G (no-reclaim baseline), 1024v/32G, 2048v/32G.
set -uo pipefail
...
run_one() {
    local vjobs_per=$1 ws_gb=$2 label=$3
    local villains=$((vjobs_per * 8))
    local per_file_sz=$(( (ws_gb << 30) / 8 ))

    echo "============================================="
    echo "=== $label: v=$villains ws=${ws_gb}G ==="
    echo "============================================="
    ...
    for t in $(seq 5 5 $DURATION); do
        ...
        echo "POLL t=$t: cpu=$cpu_psi mem=$mem_psi pgscan=+$d_scan free=${free}MB run=$run_q blk=$blk cmax=${cmax}ms"
    done
    ...
    echo ""
    echo "RESULT: $label v=$villains ws=${ws_gb}G rl=$total_rl w=$total_w max_rn=${max_rn:-0}us max_wrn=${max_wrn:-0}us cmax=${cmax}ms cpu=${cpu_psi}% mem=${mem_psi}%"
    echo ""
}

run_one 256 2 "FIX-baseline"
run_one 128 32 "FIX-1024v"
run_one 256 32 "FIX-2048v"
